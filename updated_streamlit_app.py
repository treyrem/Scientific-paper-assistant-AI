# updated_streamlit_app.py
# Updated Streamlit app with integrated figure extraction functionality
...
# Import the figure extraction module
try:
    from figure_extractor_fixed import FigureExtractor, FigureExtractionResult

    FIGURE_EXTRACTOR_AVAILABLE = True
except ImportError:
    FIGURE_EXTRACTOR_AVAILABLE = False
    logging.warning(
        "Figure extractor module not found. Make sure figure_extractor.py is in the same directory."
    )

# Import the fixed figure extraction module
try:
    from streamlit_integration_fixed import integrate_with_main_app

    FIGURE_EXTRACTOR_AVAILABLE = True
except ImportError:
    FIGURE_EXTRACTOR_AVAILABLE = False
    logging.warning(
        "Figure extractor integration module not found. Make sure streamlit_integration_fixed.py is in the same directory."
    )
...
try:
    ...
    CV2_AVAILABLE = False
    logging.warning("OpenCV not found. Please run 'pip install opencv-python'")
...
def extract_figures_from_pdf(
    ...
    """Extract figures from PDF using the FigureExtractor"""
    if not FIGURE_EXTRACTOR_AVAILABLE:
        logger.error("Figure extractor module not available.")
        return None

    try:
        # Create a figure extractor
        extractor = FigureExtractor(output_dir=output_dir)
...
if st:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 Analysis & Summary", "🖼️ Figure Extraction", "❓ Quiz", "💬 Chatbot"]
    )
        ...
        if analyze_button and uploaded_file is not None:
            if st.session_state.processed_file_name != uploaded_file.name:
                ...
                with tempfile.NamedTemporaryFile(
                    ...
                    # Save the file path for potential reuse in figure extraction
                ...
                try:
                    ...
                    # Don't delete the temp file yet, as we might need it for figure extraction
                    pass
            else:
    ...
    # --- Tab 2: Figure Extraction ---
    with tab2:
        if FIGURE_EXTRACTOR_AVAILABLE:
            integrate_with_main_app(tab2)
        else:
            st.warning(
                "Figure extraction module not available. Make sure figure_extractor_fixed.py and streamlit_integration_fixed.py are in the same directory."
            )
...
if __name__ == "__main__" and not st:
    print(
        "This script is intended to be run with Streamlit: streamlit run updated_streamlit_app.py"
    )
